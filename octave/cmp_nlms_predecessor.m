...
[y, e, a] = nlms_predecessor(x, 0.8, M);
...
subplot(423); 
plot(1:numel(y), y);#, 'Color', 'm'); #NLMS_PREDECESSOR Ausgabe
...
signal -= 127.5;
[y, e, a] = nlms_predecessor(signal, 0.8, M);

subplot(422); 
plot(1:N, signal(1:N)); hold on #Synthetisiertes Signal
xlabel(["Bildsignal"]);
subplot(424); 
plot(1:N, y(1:N));#, 'Color', 'm'); #NLMS_PREDECESSOR Ausgabe
xlabel(["Filterausgabe (M=" num2str(M) ")"]);
subplot(426); 
...
[file,path,indx] = uiputfile(['nlms_pre_m' num2str(M) '.pdf']);
