...
signal -= 127.5;
[y, e, a] = nlms_mean(signal, 0.8, M);

subplot(422); 
plot(1:N, signal(1:N)); hold on #Synthetisiertes Signal
xlabel(["Bildsignal"]);
subplot(424); 
plot(1:N, y(1:N));#, 'Color', 'm'); #NLMS Ausgabe
xlabel(["Filterausgabe (M=" num2str(M) ")"]);
subplot(426); 
...
[file,path,indx] = uiputfile(['nlms_m' num2str(M) '.pdf']);
