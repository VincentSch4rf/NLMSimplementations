...
subplot(423); 
plot(1:numel(y), y);#, 'Color', 'm'); #nlms_diff_pre Ausgabe
...
signal -= 127.5;
[y, e, a] = nlms_diff_pre(signal, 0.8, M);

subplot(422); 
plot(1:N, signal(1:N)); hold on #Synthetisiertes Signal
xlabel(["Bildsignal"]);
subplot(424); 
plot(1:N, y(1:N));#, 'Color', 'm'); #nlms_diff_pre Ausgabe
xlabel(["Filterausgabe (M=" num2str(M) ")"]);
subplot(426); 
...
[file,path,indx] = uiputfile(['nlms_diff_pre_m' num2str(M) '.pdf']);
