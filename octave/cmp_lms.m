...
for i = 1:N
  Schaetzfehler_Energie = Schaetzfehler_Energie + (e(i) .^ 2);
end
Schaetzfehler_Energie = Schaetzfehler_Energie / N
...
for i = 1:N
  Abweichungen_Energie = Abweichungen_Energie + (a_sim(i) .^ 2);
end
Abweichungen_Energie = Abweichungen_Energie/ N

%Bild
[file,img_path] = uigetfile({'*.png'; '*.jpg' }, "Bitte Bilddatei auswaehlen.");
[signal, gray] = signal_from_image(strcat(img_path, file));
signal2 = cast(signal, "double");
signal = signal2 - 127.5;
[y, e, a] = lms(signal2, 0.8, M);
